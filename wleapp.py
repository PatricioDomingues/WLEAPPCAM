...
def main():
    parser = argparse.ArgumentParser(description='WLEAPP: Windows Logs, Events, and Properties Parser.')
    ...
    #---------------------------------------------
    # Command line option "--modules/-m".
    # Allows running a specific module from the
    # command line (2025-03-16)
    #---------------------------------------------
    parser.add_argument('-m', '--modules', nargs='+', required=False, action="store",
                help=f'list the modules you would like to run, {[key for key, value in tosearch.items()]}')
    ...
    if args.artifact_paths == True:
        ...
        if not os.path.exists(output_path):
            try:
                ...
                parser.error(f"[ERROR] Cannot create OUTPUT folder '{output_path}': {e}")
                return  

        #----------------------------------------
        # Code that allows to select a specific
        # module from the command line
        #----------------------------------------
        # hardcode usagestatsVersion as first item
        search_list = {}
...
def crunch_artifacts(search_list, extracttype, input_path, out_params, ratio, wrap_text):
    ...
    logfunc('\n--------------------------------------------------------------------------------------')
    logfunc(f'WLEAPP v{wleapp_version}: Windows Logs, Events, and Properties Parser')
    logfunc('Objective: Triage Windows OS artifacts.')
